...
export default function EditTrade() {
  ...
  useEffect(() => {
    const fetchTrade = async () => {
      try {
        ...
        setExitDate(data.exitDate ? new Date(data.exitDate) : null);

        setJournal({
          preTrade: data.journal?.preTrade || "",
          postTrade: data.journal?.postTrade || "",
          emotions: data.journal?.emotions || "",
          lessons: data.journal?.lessons || "",
          tags: data.journal?.tags || "",
          rating: data.journal?.rating ?? 5,
          ...
          screenshots: data.journal?.screenshots || [],
        });
      } catch {
  ...
  const toggleChecklist = (key: string) =>
    setJournal({
      ...journal,
      checklist: { ...journal.checklist, [key]: !journal.checklist[key] },
    });
  ...
  const addCheckpoint = () => {
    ...
    setJournal({
      ...journal,
      checklist: { ...journal.checklist, [newCheckpoint]: false },
    });
    setNewCheckpoint("");
  ...
  const removeExistingScreenshot = (url: string) =>
    setJournal({
      ...journal,
      screenshots: journal.screenshots.filter((s) => s !== url),
    });

  const removeNewScreenshot = (i: number) =>
    setNewScreenshots(newScreenshots.filter((_, idx) => idx !== i));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await API.put(`/trades/${id}/journal`, formData);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-3 sm:p-4 md:p-6">
      <div className="max-w-6xl mx-auto">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Update trade details and journal
                  </p>
                </div>
              </div>
        ...
        <div className={`${activeSection === "trade" ? "block" : "hidden"} sm:block`}>
          {/* TRADE DETAILS SECTION - ADDED FROM ADDTRADE */}
        ...
        {/* JOURNAL SECTION - ADDED FROM ADDTRADE */}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-5 md:p-6 shadow-sm">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                {journal.screenshots.length > 0 && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4">
                    {journal.screenshots.map((url, i) => (
        ...
        {/* Mobile Navigation - ADDED FROM ADDTRADE */}
        ...
        {/* FOOTER ACTIONS - ADDED FROM ADDTRADE */}
